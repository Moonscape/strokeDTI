...
class KEGG:
    ...
    def _get_entry_attributes_as_dataframe(self):
        ...
        if True:
            compound_ids = pd.read_csv(
                KEGG_COMPOUND_FILE, names=["name", "aliases"], sep="\t"
            )
            compound_ids.index = compound_ids["name"].apply(lambda x: x.split(":")[1])
            compound_ids["name"] = compound_ids["aliases"].apply(
                lambda x: x.split(";")[0]
            )

            compound_rows = entry_attributes_df["name"].isin(compound_ids.index)
            entry_attributes_df.loc[compound_rows, ["name", "aliases"]] = (
                compound_ids.loc[entry_attributes_df[compound_rows]["name"]].values
            )
    ...
    def _replace_group_edges(self, edge_attributes_df):
        ...
        def update_edge_attributes(edge_attributes_df, group_rows, edge_columns):
            """
            Updates the edge_attributes_df by appending new rows from group_rows.

            Parameters:
                    edge_attributes_df (pd.DataFrame): The existing DataFrame to update.
                    group_rows (list of dict or list of lists): The new rows to append.
                    edge_columns (list of str): Column names for the new DataFrame.

            Returns:
                    pd.DataFrame: The updated DataFrame.
            """
            # Create a new DataFrame from group_rows
            new_df = pd.DataFrame(group_rows, columns=edge_columns)

            # Fill NaN values with 0
            new_df = new_df.fillna(0)

            # Concatenate the existing DataFrame with the new DataFrame
            updated_df = pd.concat([edge_attributes_df, new_df], ignore_index=True)
        ...
        for group_element in self._groups:
            ...
            edge_attributes_df = update_edge_attributes(
                edge_attributes_df, group_rows, self.edge_columns
            )
    ...
    def output_KGML_as_directed_networkx(self, genes_only=True):
        ...
        nx.relabel_nodes(
            graph,
            {node_id: graph.node[node_id]["name"] for node_id in graph.nodes()},
            copy=False,
        )
    ...
    def view(self, scale=1, show_compounds=False, gene_values=None):
        ...
        if gene_values is not None:
            ...
            gene_colors = {
                node: hex_lookup[idx] for node, idx in rgb_indices.iteritems()
            }
        ...
        for _, edge_attribs in self.edge_attributes_df.iterrows():
            ...
            arrow = ax.annotate(
                "", xy=target_pos, xytext=source_pos, arrowprops=arrowprops
            )
